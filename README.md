...
It runs locally over stdio, so the agent keeps working inside the user's machine and repository context. This makes it suitable for MCP-compatible agents such as Claude Desktop, Cursor, Cline, OpenCode, and other clients that can launch local MCP servers.
